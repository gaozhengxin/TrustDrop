...
pub const PROBE_VSS_SHAPE_ELF: Elf = include_elf!("probe-vss-shape-program");
...
async fn main() {
    ...
    let pk = client.setup(PROBE_VSS_SHAPE_ELF).await.unwrap();
    println!("probe-vss-shape sdk=6.2.4 network=mainnet");
    println!("probe-vss-shape vkey={}", pk.verifying_key().bytes32());
...
fn write_fixture(
    ...
    std::fs::write(
        fixture_dir.join("probe-vss-shape-groth16-fixture.json"),
        serde_json::to_string_pretty(&fixture).unwrap(),
    )
    .expect("failed to write fixture");
    println!("probe-vss-shape fixture saved");
}
