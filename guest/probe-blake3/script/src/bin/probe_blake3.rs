...
pub const PROBE_BLAKE3_ELF: Elf = include_elf!("probe-blake3-program");
...
async fn main() {
    ...
    let pk = client.setup(PROBE_BLAKE3_ELF).await.unwrap();
    println!("probe-blake3 sdk=6.2.4 network=mainnet");
    println!("probe-blake3 vkey={}", pk.verifying_key().bytes32());
...
fn write_fixture(n: u32, proof: &SP1ProofWithPublicValues, vk: &SP1VerifyingKey) {
    ...
    std::fs::write(
        fixture_dir.join("probe-blake3-groth16-fixture.json"),
        serde_json::to_string_pretty(&fixture).unwrap(),
    )
    .expect("failed to write fixture");
    println!("probe-blake3 fixture saved");
}
