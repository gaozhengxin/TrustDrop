...
fn main() {
    ...
    let c_cipher = compute_ipfs_cid_zk_optimized(&cipher);

    let mut c_cipher_str = "".into();
    if let Ok(cid_obj) = cid::Cid::read_bytes(c_cipher.as_slice()) {
        c_cipher_str = cid_obj.to_string();
    } else {
        eprintln!(" Invalid CID Binary 0x{}", hex::encode(&c_cipher));
    }

    // combined expected public output (what guest commit_slice will commit)
    let mut combined_expected: Vec<u8> = Vec::with_capacity(32 + 32 + c_cipher.len() + 4);
    ...
    if args.execute {
        // Execute the program
        let (output, report) = client.execute(VDD_FILECOIN_ELF, &stdin).run().unwrap();
        ...
        if output.as_slice() == combined_expected.as_slice() {
            ...
            let show = (128).min(output.as_slice().len());
            println!(
        ...
        let (output, _report) = client.execute(VDD_FILECOIN_ELF, &stdin).run().unwrap();
